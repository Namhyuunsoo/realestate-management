...
function applyAllFilters() {
  ...
  */
  ...
  const NUM_CONFIG = UI_STATE.listingMode === "housing"
    ...
    : { area_sale: "gte", area_real: "gte", deposit: "lte", rent: "lte", premium: "lte" };
  ...
  let arr = LISTINGS.filter(item => {
    ...
    for (const tk of TEXT_KEYS) {
      const fieldName = FIELDS[tk];
      if (!fieldName) continue;
      const v = fields[fieldName] || "";
      if (!matchesTextTokens(v, parsedText[tk])) return false;
    }

    const floorField = FIELDS.floor;
    if (floorField) {
      const fVal = parseFloorValue(fields[floorField]);
      if (!checkNumFilter(fVal, floorFilter)) return false;
    }

    for (const nk of Object.keys(NUM_CONFIG)) {
      const fieldName = FIELDS[nk];
      if (!fieldName) continue;
      const val = parseNumber(fields[fieldName]);
