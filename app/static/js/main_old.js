...
// adjustLayoutHeight 함수는 setLayoutHeight와 동일하므로 제거
...
function setLayoutHeight() {
  ...
  let usedHeight = 0;
  if (topbar) usedHeight += topbar.offsetHeight;
  if (statusCounts) usedHeight += statusCounts.offsetHeight;
  if (topFilterBar) usedHeight += topFilterBar.offsetHeight;
  
  const layout = document.getElementById("layout");
  if (layout) {
    layout.style.height = `${vh - usedHeight}px`;
  }
}
...
async function syncUserFromSession() {
  try {
    ...
    if (res.ok && data && data.email) {
      ...
      if (usEl) usEl.textContent = `로그인: ${data.email}`;
...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  function showSecondaryPanel(viewId) {
    ...
    // CSS transform을 사용하여 표시 (기존 CSS 로직 활용)
  ...
  // 중복된 이벤트 리스너 제거 - 위쪽에서 이미 등록됨

  // 동적으로 생성되는 버튼들의 이벤트는 각각의 렌더링 함수에서 처리됨
  // (renderCustomerForm, renderCustomerEditForm에서 처리)
  ...
  // 2차 사이드바 닫기 버튼은 위쪽에서 이미 처리됨

  // 정렬 버튼 초기화 및 이벤트 등록
  const sortButtons = document.querySelectorAll(".sortBtn");
  
  // 정렬 버튼 초기 상태 설정 (텍스트는 변경하지 않음)
  ...
  console.log("✅ 초기화 완료 - 모든 패널 숨김, 로그인 상태:", !!currentUser);
});

// 클러스터 클릭 위임 바인딩은 메인 DOMContentLoaded에서 처리됨 

// 고객 컨트롤 UI 리팩토링 (2번 사이드바에서 목록+상세 세로 배치)
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const viewCustomerForm = document.getElementById('viewCustomerForm');
  const viewCustomerDetail = document.getElementById('viewCustomerDetail');
  const customerListContent = document.getElementById('customerListContent2');
  
  // DOM 요소 확인 (디버그용)
  dbg('DOM 요소 확인:', {
    customerListBtn, newCustomerBtn, secondaryPanel, 
    detailTitleEl, viewCustomerList, viewCustomerForm, viewCustomerDetail
  });

  // 상태 초기화 함수는 위쪽에 이미 정의됨
  ...
  window.afterCustomerSaved = function() {
    hideAllSecondaryViews();
    showSecondaryPanel('viewCustomerList');
    detailTitleEl.textContent = '내 고객 목록';
    loadCustomerList(currentUser === 'admin' ? 'all' : 'own');
  };
  
});

// submitCustomerForm 함수 오버라이드 제거 - 원본 함수에서 직접 처리
...
// 중복된 함수 정의 제거 - 위쪽에 이미 정의됨

// 중복된 함수 정의 제거 - 위쪽에 이미 정의됨

// 닫기 버튼 이벤트는 메인 DOMContentLoaded에서 처리됨
